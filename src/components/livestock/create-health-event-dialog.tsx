...
import { createHealthEvent } from "@/app/actions/health-actions"
...
interface CreateHealthEventDialogProps {
    batchId: string
    batchName: string
}

export function CreateHealthEventDialog({ batchId, batchName }: CreateHealthEventDialogProps) {
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        ...
        const result = await createHealthEvent(null, formData)

        if (result?.success) {
            setOpen(false)
            form.reset()
            toast.success("Événement santé enregistré avec succès")
        } else {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Santé - {batchName}</DialogTitle>
                    <DialogDescription>
                        Enregistrez un vaccin, un traitement ou une maladie.
                    </DialogDescription>
                </DialogHeader>
                <Form {...form}>
