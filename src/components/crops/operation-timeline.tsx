                    ...
                    <div key={op.id} className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                        ...
                        <div className="w-[calc(100%-4rem)] md:w-[calc(50%-2.5rem)] p-4 rounded border border-slate-200 shadow">
                            <div className="flex items-center justify-between mb-1">
                                <div className="font-bold text-slate-900">{op.description}</div>
                                <time className="font-caveat font-medium text-emerald-600">
                                    {format(op.date, "d MMM yyyy", { locale: fr })}
                                </time>
                            </div>
                            <div className="text-slate-500 text-sm">
                                {op.type} • {op.status === 'COMPLETED' ? 'Terminé' : 'Prévu'}
                                {op.cost && <span className="ml-2 text-slate-700 font-semibold">{op.cost.toLocaleString()} FCFA</span>}
                            </div>
                            {op.notes && (
